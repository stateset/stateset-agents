# GLM 5.2 Hosting Plan

This document defines the minimum viable path to host
[`zai-org/GLM-5.2`](https://huggingface.co/zai-org/GLM-5.2) on the current
StateSet Agents stack and expose an endpoint that accepts API calls.

GLM 5.2 is a 754B-parameter Mixture-of-Experts reasoning model from Zhipu AI
...
| Model | Repo | Notes |
| --- | --- | --- |
| GLM-5.2 | `zai-org/GLM-5.2` | BF16 weights, ~1.5 TB on disk |
| GLM-5.2-FP8 | `your-org/GLM-5.2-FP8` | Example private FP8 deployment alias, ~754 GB on disk |
...
- Public model name at the gateway: `zai-org/GLM-5.2`
- Private serving model in `vLLM`: `your-org/GLM-5.2-FP8`
...
| --- | --- | --- | --- |
| Single-host (FP8) | `your-org/GLM-5.2-FP8` | 1x 8xH200 (1128 GB VRAM) | `--tensor-parallel-size=8 --quantization=fp8` |
| Multi-node (BF16) | `zai-org/GLM-5.2` | 2x 8xH100 / 8xH200 | `--tensor-parallel-size=8 --pipeline-parallel-size=2` |

The single-host FP8 path is the minimum practical setup with the current Helm
chart and is what `values-glm5-2-fp8.yaml` targets.
...
2. The StateSet gateway exposes `POST /v1/messages`, `POST /v1/chat/completions`,
   and `GET /v1/models`.
3. The gateway rewrites the public model name `zai-org/GLM-5.2` to your
   private FP8 model id `your-org/GLM-5.2-FP8` via `INFERENCE_MODEL_MAP`.
...
helm upgrade --install stateset-agents \
  ...
  -f deployment/helm/stateset-agents/values-glm5-2-fp8.yaml
```

Or apply the raw manifest:

```bash
kubectl apply -f deployment/kubernetes/glm5-2-vllm-fp8.yaml
```
...
helm upgrade --install stateset-agents \
  ./deployment/helm/stateset-agents \
  -n stateset-agents --create-namespace \
  -f deployment/helm/stateset-agents/values-glm5-2.yaml
```

Or:

```bash
kubectl apply -f deployment/kubernetes/glm5-2-vllm.yaml
```
...
helm upgrade --install stateset-agents \
  ./deployment/helm/stateset-agents \
  -n stateset-agents --create-namespace \
  -f deployment/helm/stateset-agents/values-glm5-2-finetuned.yaml
```

The matching K8s manifests are in
`deployment/kubernetes/glm5-2-vllm-finetuned.yaml` and
`deployment/kubernetes/glm5-2-vllm-finetuned-gcs.yaml` (the GCS variant
syncs merged weights from a Cloud Storage bucket at pod startup).

## 5. Fine-tuning

GLM 5.2 fine-tuning is QLoRA-only on this stack — full fine-tuning of a 754B
MoE is not feasible. The dedicated starter is
[`examples/finetune_glm5_2_gspo.py`](../examples/finetune_glm5_2_gspo.py),
which wraps `stateset_agents.training.glm5_2_starter` and emits a
...
python examples/finetune_glm5_2_gspo.py --write-config ./glm5_2.json

# Real run on a multi-node cluster
python examples/finetune_glm5_2_gspo.py --no-dry-run --task customer_service
```

The Kubernetes Job manifest at
[`deployment/kubernetes/glm5-2-training-job.yaml`](../deployment/kubernetes/glm5-2-training-job.yaml)
runs this script under the StateSet trainer image with a PVC-backed
`/models/glm5-2` output directory.
...
python scripts/render_glm5_2_helm_values.py \
  --manifest /models/glm5-2/serving_manifest.json \
...
python scripts/render_glm5_2_helm_values.py \
  --manifest /models/glm5-2/serving_manifest.json \
  > deployment/helm/stateset-agents/values-glm5-2-run-123.yaml
```
...
| `--max-model-len` | `131072` | GLM 5.2 supports up to ~200K, this leaves KV headroom |
...
`stateset_agents.training.glm5_2_starter.get_glm5_2_serving_recommendations`
...
2. `kubectl port-forward svc/glm5-2-vllm-fp8 8000:8000` and `curl
   localhost:8000/v1/models` returns your FP8 alias (for example
   `your-org/GLM-5.2-FP8`).
...
- Training module: [glm5_2_starter.py](../stateset_agents/training/glm5_2_starter.py)
- Example script: [finetune_glm5_2_gspo.py](../examples/finetune_glm5_2_gspo.py)
- Helm values: [values-glm5-2.yaml](../deployment/helm/stateset-agents/values-glm5-2.yaml),
  [values-glm5-2-fp8.yaml](../deployment/helm/stateset-agents/values-glm5-2-fp8.yaml),
  [values-glm5-2-finetuned.yaml](../deployment/helm/stateset-agents/values-glm5-2-finetuned.yaml)
- K8s manifests: [glm5-2-vllm.yaml](../deployment/kubernetes/glm5-2-vllm.yaml),
  [glm5-2-vllm-fp8.yaml](../deployment/kubernetes/glm5-2-vllm-fp8.yaml),
  [glm5-2-vllm-finetuned.yaml](../deployment/kubernetes/glm5-2-vllm-finetuned.yaml),
  [glm5-2-vllm-finetuned-gcs.yaml](../deployment/kubernetes/glm5-2-vllm-finetuned-gcs.yaml),
  [glm5-2-training-job.yaml](../deployment/kubernetes/glm5-2-training-job.yaml)
- Helm renderer: [render_glm5_2_helm_values.py](../scripts/render_glm5_2_helm_values.py)
