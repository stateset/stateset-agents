...
| River AI provider | **Live-verified** (2026‑08‑18) | `train-remote --provider river` trained Qwen3.5‑9B for real (session → LoRA model → train step → `river://` checkpoint, lineage manifest written) and `chat_complete_from_checkpoint` answered from the trained weights. Their SDK landed on PyPI 2026‑08‑17; Python ≥3.12 (separate venv — the repo runs 3.10) |
