...
| Tool calls are verified deterministically | Unit-pinned (2026‑08‑19); first live run pending | fenced-json actions matched on tool name + args subset; prose that merely claims the action fails (`check_tool_call`, ladder `Issue.tool`, episode `turn_tool`) |
| The rarity controller targets the operating window | Unit-pinned (2026‑08‑19); first live run pending | `--target-harvest-rate`: per-generation temperature probes pick the harvest nearest the measured ~60% window |
