...
class TestAgentPerformance:
    ...
    async def test_agent_initialization_performance(
        ...
        print(
            f"Agent initialization: {init_time:.3f}s, Memory delta: {memory_delta:.1f}MB"
        )
    ...
    async def test_response_generation_performance(
        ...
        assert (
            response_time < 2.0
        ), f"Response generation took too long: {response_time}s"
    ...
    async def test_reward_computation_performance(self, performance_reward):
        ...
        with patch.object(
            ...
            assert (
                avg_time < 0.1
            ), f"Reward computation too slow: {avg_time:.3f}s per call"
            assert result == 0.85
...
class TestBatchOperationsPerformance:
    ...
    async def test_batch_response_generation(self, mock_tokenizer, mock_model):
        ...
        print(
            f"Batch processing (5 responses): {total_time:.3f}s total, {avg_time:.3f}s average"
        )
    ...
    async def test_memory_usage_during_conversation(self, mock_tokenizer, mock_model):
        ...
        for turn in range(10):
            messages = [
                {
                    "role": "user",
                    "content": f"Turn {turn + 1}: Tell me something interesting.",
                }
            ]
            ...
            if turn % 3 == 0:
                ...
                assert (
                    memory_delta < 100
                ), f"Memory leak detected: {memory_delta:.1f}MB increase at turn {turn + 1}"

        final_memory = process.memory_info().rss / 1024 / 1024
        total_memory_delta = final_memory - initial_memory

        print(
            f"Memory usage: Initial {initial_memory:.1f}MB, Final {final_memory:.1f}MB, Delta {total_memory_delta:.1f}MB"
        )

        # Overall memory growth should be reasonable
        assert (
            total_memory_delta < 50
        ), f"Excessive memory growth: {total_memory_delta:.1f}MB"
