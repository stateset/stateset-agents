...
class TestTrainingPipelineIntegration:
    ...
    async def test_full_training_pipeline(self, mock_tokenizer, mock_model, 
                                        agent_config, conversation_scenarios, training_config):
    ...
    @patch('stateset_agents.core.agent.AutoModelForCausalLM')
    @patch('stateset_agents.core.agent.AutoTokenizer')
    async def test_environment_agent_interaction(self, mock_tokenizer, mock_model,
                                                agent_config, conversation_scenarios):
        ...
        for step in range(2):
            ...
            messages = [
                {"role": "user", "content": f"Test message {step + 1}"}
            ]
    ...
    async def test_reward_function_integration(self, agent_config):
        ...
        with patch.object(reward_fn, 'compute_reward') as mock_compute:
            ...
            turns = [
                ...
                {"role": "assistant", "content": "Start with the official tutorial at python.org"}
            ]
    ...
    @patch('stateset_agents.core.agent.AutoModelForCausalLM')
    @patch('stateset_agents.core.agent.AutoTokenizer')
    async def test_agent_persistence_workflow(self, mock_tokenizer, mock_model, agent_config, tmp_path):
