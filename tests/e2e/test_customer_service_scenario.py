...
class TestCustomerServiceE2E:
    ...
    async def test_complete_customer_service_workflow(self, mock_tokenizer, mock_model,
                                                    customer_service_config, customer_service_scenarios):
        ...
        with patch.object(reward_fn, 'compute_reward', return_value=AsyncMock(return_value=0.8)):
        ...
        test_conversation = [
            {"role": "user", "content": "Hi, I can't access my account. I think I forgot my password."}
        ]
        ...
        assert any(keyword in response.lower() for keyword in 
                  ["password", "reset", "help", "account", "email"])
        
        # Step 6: Test conversation continuity
        conversation_history = [
            {"role": "user", "content": "I forgot my password"},
            {"role": "assistant", "content": response},
            {"role": "user", "content": "I checked my email but didn't see the reset link"}
        ]
    ...
    @patch('stateset_agents.core.agent.AutoModelForCausalLM')
    @patch('stateset_agents.core.agent.AutoTokenizer')
    async def test_agent_adaptation_to_different_personas(self, mock_tokenizer, mock_model, customer_service_config):
        ...
        for test_case in test_cases:
            ...
            has_expected_keyword = any(keyword in response_lower 
                                     for keyword in test_case["expected_keywords"])
            assert has_expected_keyword, f"Response '{response}' doesn't match expected keywords {test_case['expected_keywords']}"
    
    @pytest.mark.asyncio
    @patch('stateset_agents.core.agent.AutoModelForCausalLM')
    @patch('stateset_agents.core.agent.AutoTokenizer')
    async def test_performance_under_load(self, mock_tokenizer, mock_model, customer_service_config):
        ...
        mock_tokenizer_instance.eos_token_id = 2
        mock_tokenizer_instance.apply_chat_template.return_value = [1, 2, 3]
        mock_tokenizer_instance.decode.return_value = "Thank you for your question. I'm here to help!"
        ...
        async def simulate_conversation(conversation_id: int):
            messages = [{"role": "user", "content": f"Question from customer {conversation_id}"}]
            response = await agent.generate_response(messages)
            return response
