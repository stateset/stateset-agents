"""Unit tests for the GLM 5.2 starter configuration."""
...
class TestGlm52Config:
    """Test suite for the GLM 5.2 helper config."""

    def test_default_config_creation(self):
        config = Glm52Config()

        assert config.model_name == GLM5_2_BASE_MODEL
    ...
    def test_memory_profile_defaults(self):
        config = get_glm5_2_config(starter_profile="memory")
    ...
    def test_quality_profile_defaults(self):
        config = get_glm5_2_config(starter_profile="quality")
        profile = get_glm5_2_profile_overrides("quality")
    ...
    def test_config_with_custom_task(self):
        config = get_glm5_2_config(task="technical_support", num_outer_iterations=15)
    ...
    def test_config_with_lora_disabled(self):
        config = get_glm5_2_config(use_lora=False)
    ...
    def test_quantization_flags(self):
        config = get_glm5_2_config(use_4bit=True, use_8bit=True)
    ...
    def test_agent_config_creation(self):
        config = get_glm5_2_config()
        agent_config = create_glm5_2_agent_config(config)

        assert agent_config.model_name == GLM5_2_BASE_MODEL
        assert agent_config.trust_remote_code is True
        assert agent_config.max_new_tokens == 1536

    def test_config_summary(self):
        config = get_glm5_2_config(starter_profile="memory")
        summary = summarize_glm5_2_config(config)
    ...
    def test_gspo_config_generation(self):
        base_config = get_config_for_task(
            "customer_service", model_name=GLM5_2_BASE_MODEL
        )
        config = get_glm5_2_config(task="customer_service")

        with patch(
            "stateset_agents.training.glm5_2_starter.get_config_for_task",
            return_value=base_config,
        ):
            gspo_config = get_glm5_2_gspo_config(config)

        assert gspo_config.model_name == GLM5_2_BASE_MODEL
        assert gspo_config.use_lora is True
        assert gspo_config.use_vllm is True
        assert gspo_config.use_4bit is True
        assert gspo_config.lora_target_modules == GLM5_2_LORA_TARGET_MODULES
    ...
    def test_validation_warnings(self):
        ...
        warnings = validate_glm5_2_config(config)
    ...
    def test_serving_recommendations_fp8(self):
        rec = get_glm5_2_serving_recommendations(use_fp8=True)
    ...
    def test_serving_recommendations_disable_tool_choice(self):
        rec = get_glm5_2_serving_recommendations(enable_auto_tool_choice=False)
    ...
    def test_json_config_file_roundtrip(self, tmp_path):
        config = get_glm5_2_config(
            ...
            use_lora=True,
            learning_rate=2e-6,
            output_dir="./outputs/glm5_2_roundtrip",
        )
        config_path = write_glm5_2_config_file(config, tmp_path / "glm5_2.json")
        loaded = load_glm5_2_config_file(config_path)

        assert loaded.task == "technical_support"
        assert loaded.use_lora is True
        assert loaded.learning_rate == 2e-6
        assert loaded.output_dir == "./outputs/glm5_2_roundtrip"

    def test_preview_payload_can_be_loaded_as_config(self, tmp_path):
        config = get_glm5_2_config(task="sales", output_dir="./outputs/glm5_2_preview")
        preview_path = write_glm5_2_config_file(
            config,
            tmp_path / "glm5_2_preview.json",
            include_preview=True,
        )
        loaded = load_glm5_2_config_file(preview_path)
        preview = create_glm5_2_preview(loaded)

        assert loaded.task == "sales"
        assert preview["config"]["output_dir"] == "./outputs/glm5_2_preview"

    def test_examples_helper_reexports_packaged_symbols(self):
        from examples import glm5_2_config as example_config_module

        assert example_config_module.Glm52Config is Glm52Config
        assert example_config_module.GLM5_2_BASE_MODEL == GLM5_2_BASE_MODEL
        assert example_config_module.GLM5_2_FP8_MODEL == GLM5_2_FP8_MODEL
        assert (
            example_config_module.GLM5_2_STARTER_PROFILE_CHOICES
            == GLM5_2_STARTER_PROFILE_CHOICES
        )
        assert (
            example_config_module.describe_glm5_2_starter_profiles
            is describe_glm5_2_starter_profiles
        )
        assert (
            example_config_module.get_glm5_2_serving_recommendations
            is get_glm5_2_serving_recommendations
        )


class TestGlm52StarterScript:
    """Test the dedicated GLM 5.2 starter script surface."""

    def test_training_function_signature(self):
        from examples import finetune_glm5_2_gspo as training_module

        sig = inspect.signature(training_module.finetune_glm5_2)
    ...
    def test_dry_run_preview_payload(self):
        from examples.finetune_glm5_2_gspo import create_glm5_2_dry_run_payload

        preview = create_glm5_2_dry_run_payload()

        assert preview["model_name"] == GLM5_2_BASE_MODEL
    ...
    def test_dry_run_preview_payload_fp8(self):
        ...
        preview = create_glm5_2_dry_run_payload(
            model_name=GLM5_2_FP8_MODEL, use_fp8_serving=True
        )

        assert preview["model_name"] == GLM5_2_FP8_MODEL
    ...
    def test_async_dry_run(self):
        from stateset_agents.training.glm5_2_starter import finetune_glm5_2

        preview = asyncio.run(finetune_glm5_2(dry_run=True))

        assert preview["config"]["model_name"] == GLM5_2_BASE_MODEL
        assert preview["summary"]["starter_profile"] == "balanced"
    ...
    def test_cli_dry_run_subprocess(self):
        ...
        assert payload["model_name"] == GLM5_2_BASE_MODEL
