...
def test_cli_kimi_k3_dry_run_json():
    """Test the dedicated Kimi-K3 starter preview payload."""
    result = runner.invoke(app, ["kimi-k3", "--json-output"])
    assert result.exit_code == 0
    payload = json.loads(result.stdout)
    assert payload["config"]["model_name"] == "moonshotai/Kimi-K3"
    assert payload["config"]["task"] == "customer_service"
    assert payload["config"]["starter_profile"] == "balanced"
    assert payload["gspo_overrides"]["use_lora"] is True
    assert payload["gspo_overrides"]["output_dir"] == "./outputs/kimi_k3_gspo"


def test_cli_kimi_k3_memory_profile_json():
    """Test the low-memory Kimi starter profile."""
    result = runner.invoke(
        app,
        ["kimi-k3", "--starter-profile", "memory", "--json-output"],
    )
    assert result.exit_code == 0
    payload = json.loads(result.stdout)
    assert payload["config"]["starter_profile"] == "memory"
...
def test_cli_kimi_k3_list_profiles_json():
    """Test the starter can describe all built-in Kimi profiles."""
    result = runner.invoke(
        app,
        ["kimi-k3", "--task", "sales", "--list-profiles", "--json-output"],
    )
    assert result.exit_code == 0
    payload = json.loads(result.stdout)
    assert payload["task"] == "sales"
...
def test_cli_kimi_k3_list_profiles_text():
    """Test the human-readable Kimi profile catalog output."""
    result = runner.invoke(app, ["kimi-k3", "--list-profiles"])
    assert result.exit_code == 0
    assert "Available Kimi-K3 starter profiles:" in result.stdout
    assert "- balanced:" in result.stdout
    assert "- memory:" in result.stdout
    assert "- quality:" in result.stdout
    assert "quantization=4bit" in result.stdout
...
def test_cli_kimi_k3_rejects_unknown_task():
    """Test the dedicated Kimi starter rejects unsupported task names."""
    result = runner.invoke(app, ["kimi-k3", "--task", "unknown_task"])
    assert result.exit_code != 0
    assert "unsupported task" in result.stdout.lower()


def test_cli_kimi_k3_rejects_unknown_starter_profile():
    """Test the dedicated Kimi starter rejects unsupported profile names."""
    result = runner.invoke(app, ["kimi-k3", "--starter-profile", "unknown_profile"])
    assert result.exit_code != 0
    assert "unsupported starter profile" in result.stdout.lower()


def test_cli_kimi_k3_write_config_json(tmp_path):
    """Test the dedicated Kimi starter can write a reusable config file."""
    cfg_path = tmp_path / "kimi_k3.json"
    result = runner.invoke(app, ["kimi-k3", "--write-config", str(cfg_path)])
    assert result.exit_code == 0
    assert cfg_path.exists()
    loaded = json.loads(cfg_path.read_text(encoding="utf-8"))
    assert loaded["model_name"] == "moonshotai/Kimi-K3"
    assert loaded["task"] == "customer_service"


def test_cli_kimi_k3_load_config_json(tmp_path):
    """Test the dedicated Kimi starter can load a saved config file."""
    cfg_path = tmp_path / "kimi_k3_custom.json"
    cfg_path.write_text(
        json.dumps(
            {
                "model_name": "moonshotai/Kimi-K3",
                "task": "sales",
                "output_dir": "./outputs/kimi_k3_loaded",
                "learning_rate": 1e-5,
                "use_lora": False,
            },
            indent=2,
    ...
    result = runner.invoke(
        app,
        ["kimi-k3", "--config", str(cfg_path), "--json-output"],
    )
    assert result.exit_code == 0
    payload = json.loads(result.stdout)
    assert payload["config"]["task"] == "sales"
    assert payload["config"]["output_dir"] == "./outputs/kimi_k3_loaded"
    assert payload["gspo_overrides"]["use_lora"] is False
...
def test_cli_init_kimi_k3_preset_json(tmp_path):
    """Test init can scaffold the Kimi-K3 starter preset."""
    cfg_path = tmp_path / "kimi_k3.json"
    result = runner.invoke(
        app,
        [
...
def test_cli_init_kimi_k3_preset_custom_task(tmp_path):
    """Test init can scaffold the Kimi starter preset for another task."""
    cfg_path = tmp_path / "kimi_k3_sales.json"
    result = runner.invoke(
        app,
        [
            "init-config",
...
def test_cli_init_kimi_k3_preset_memory_profile_json(tmp_path):
    """Test init can scaffold the low-memory Kimi starter preset."""
    cfg_path = tmp_path / "kimi_k3_memory.json"
    result = runner.invoke(
        app,
        [
            "init",
