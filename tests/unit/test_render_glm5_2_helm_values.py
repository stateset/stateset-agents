...
from scripts.render_glm5_2_helm_values import build_values_from_manifest
...
def test_render_values_from_manifest_finetuned(tmp_path):
    manifest = {
        ...
        "merged_model_dir": "/models/glm5-2/merged",
        "recommended": {
    ...
    assert 'INFERENCE_DEFAULT_MODEL: "zai-org/GLM-5.2"' in rendered
    assert (
        'INFERENCE_MODEL_MAP: "{\\"zai-org/GLM-5.2\\": \\"/models/glm5-2/merged\\"}"'
        in rendered
    )
    assert 'modelPath: "/models/glm5-2/merged"' in rendered
...
def test_render_values_from_manifest_fp8(tmp_path):
    ...
    assert 'modelId: "your-org/GLM-5.2-FP8"' in rendered
...
def test_render_values_from_manifest_with_gcs_uri(tmp_path):
    manifest = {
        "model": "zai-org/GLM-5.2",
        "merged_model_dir": "/models/glm5-2/runs/run-123/merged",
        "recommended": {
            "tensor_parallel_size": 8,
            "pipeline_parallel_size": 2,
            "max_model_len": 131072,
    ...
    rendered = _run_render(
        str(path),
        "--gcs-uri",
        "gs://YOUR_BUCKET/glm5-2/runs/YOUR_RUN_ID/merged",
    )

    assert "modelSync:" in rendered
    assert 'gcsUri: "gs://YOUR_BUCKET/glm5-2/runs/YOUR_RUN_ID/merged"' in rendered
    assert 'localDir: "/models/glm5-2/runs/run-123/merged"' in rendered
...
def test_build_values_gcs_uri_flips_finetuned_without_merged_dir():
    ...
    assert values["vllm"]["modelPath"] == "/models/glm5-2/merged"
