...
class TestDeepseekV4StarterScript:
    ...
    def test_script_is_a_forwarder(self):
        ...
        examples/finetune_gspo.py --model deepseek-v4. It prints a notice
        to stderr and exits 0 under --dry-run."""
        repo_root = Path(__file__).resolve().parents[2]
        result = subprocess.run(
            [sys.executable, "examples/finetune_deepseek_v4_gspo.py", "--dry-run"],
            cwd=repo_root,
            capture_output=True,
            text=True,
            encoding="utf-8",
        ...
        assert "deepseek-v4" in result.stderr
