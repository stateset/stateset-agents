"""Unit tests for the Muse Glimmer starter configuration."""
...
class TestMuseGlimmerConfig:
    """Test suite for the Muse Glimmer helper config."""

    def test_default_config_creation(self):
        config = MuseGlimmerConfig()

        assert config.model_name == MUSE_GLIMMER_BASE_MODEL
    ...
    def test_memory_profile_defaults(self):
        config = get_muse_glimmer_config(starter_profile="memory")
    ...
    def test_quality_profile_defaults(self):
        config = get_muse_glimmer_config(starter_profile="quality")
        profile = get_muse_glimmer_profile_overrides("quality")
    ...
    def test_profile_description_lookup(self):
        description = get_muse_glimmer_profile_description("memory")

        assert description == MUSE_GLIMMER_STARTER_PROFILE_DESCRIPTIONS["memory"]
        assert "Lower-memory" in description

    def test_profile_catalog_lists_all_profiles(self):
        payload = describe_muse_glimmer_starter_profiles(task="sales")

        assert payload["task"] == "sales"
        assert payload["default_profile"] == "balanced"
        assert set(payload["profiles"]) == set(MUSE_GLIMMER_STARTER_PROFILE_CHOICES)
    ...
    def test_config_with_lora_disabled(self):
        config = get_muse_glimmer_config(use_lora=False)
    ...
    def test_quantization_flags(self):
        config = get_muse_glimmer_config(use_4bit=True, use_8bit=True)
    ...
    def test_agent_config_creation(self):
        ...
        assert agent_config.model_name == MUSE_GLIMMER_BASE_MODEL
    ...
    def test_gspo_config_generation(self):
        base_config = get_config_for_task(
            "customer_service", model_name=MUSE_GLIMMER_BASE_MODEL
        )
        config = get_muse_glimmer_config(task="customer_service")

        with patch(
            "stateset_agents.training.muse_glimmer_starter.get_config_for_task",
            return_value=base_config,
        ):
            gspo_config = get_muse_glimmer_gspo_config(config)

        assert gspo_config.model_name == MUSE_GLIMMER_BASE_MODEL
        ...
        assert gspo_config.lora_target_modules == MUSE_GLIMMER_LORA_TARGET_MODULES
    ...
    def test_validation_warnings(self):
        ...
        warnings = validate_muse_glimmer_config(config)
    ...
    def test_validation_warns_on_unknown_starter_profile(self):
        config = MuseGlimmerConfig(starter_profile="custom-profile")
        warnings = validate_muse_glimmer_config(config)

        assert any("starter_profile" in warning for warning in warnings)

    def test_validation_warns_on_non_glimmer_model(self):
        config = MuseGlimmerConfig(model_name="Qwen/Qwen3.5-0.8B")
        warnings = validate_muse_glimmer_config(config)

        assert any("muse glimmer checkpoint" in warning.lower() for warning in warnings)
    ...
    def test_json_config_file_roundtrip(self, tmp_path):
        config = get_muse_glimmer_config(
            ...
            use_lora=False,
            learning_rate=1e-5,
            output_dir="./outputs/muse_glimmer_roundtrip",
        )
        config_path = write_muse_glimmer_config_file(
            config, tmp_path / "muse_glimmer.json"
        )
        loaded = load_muse_glimmer_config_file(config_path)

        assert loaded.task == "technical_support"
        assert loaded.use_lora is False
        assert loaded.learning_rate == 1e-5
        assert loaded.output_dir == "./outputs/muse_glimmer_roundtrip"

    def test_preview_payload_can_be_loaded_as_config(self, tmp_path):
        config = get_muse_glimmer_config(
            task="sales", output_dir="./outputs/muse_glimmer_preview"
        )
        preview_path = write_muse_glimmer_config_file(
            config,
            tmp_path / "muse_glimmer_preview.json",
            include_preview=True,
        )
        loaded = load_muse_glimmer_config_file(preview_path)
        preview = create_muse_glimmer_preview(loaded)

        assert loaded.task == "sales"
        assert preview["config"]["output_dir"] == "./outputs/muse_glimmer_preview"


class TestMuseGlimmerStarterScript:
    """Test the dedicated starter script surface."""

    def test_script_is_a_forwarder(self):
        """finetune_muse_glimmer_gspo.py is a thin forwarder onto
        examples/finetune_gspo.py --model muse-glimmer. It prints a notice
        to stderr and exits 0 under --dry-run."""
        repo_root = Path(__file__).resolve().parents[2]
        result = subprocess.run(
            [sys.executable, "examples/finetune_muse_glimmer_gspo.py", "--dry-run"],
            cwd=repo_root,
            capture_output=True,
            text=True,
            encoding="utf-8",
        ...
        assert "muse-glimmer" in result.stderr
    ...
    def test_dry_run_preview_via_starter_profile(self):
        ...
        exit_code = finetune_gspo.main(
            ["--model", "muse-glimmer", "--starter-profile", "balanced", "--dry-run"]
        )
        assert exit_code == 0
    ...
    def test_cli_dry_run_subprocess(self):
        ...
        assert payload["config"]["model_name"] == MUSE_GLIMMER_BASE_MODEL
