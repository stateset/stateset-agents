...
class TestGemma4StarterScript:
    ...
    def test_script_is_a_deprecated_forwarder(self):
        """finetune_gemma4_31b_gspo.py was converted to a thin forwarder onto
        examples/finetune_gspo.py --model gemma4-31b once the driver's
        ...
        result = subprocess.run(
            [sys.executable, "examples/finetune_gemma4_31b_gspo.py", "--dry-run"],
            cwd=repo_root,
            capture_output=True,
            text=True,
            encoding="utf-8",
        ...
        assert "gemma4-31b" in result.stderr
    ...
    def test_dry_run_preview_via_starter_profile(self):
        ...
        exit_code = finetune_gspo.main(
            ["--model", "gemma4-31b", "--starter-profile", "balanced", "--dry-run"]
        )
        assert exit_code == 0
