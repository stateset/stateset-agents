...
install-locked: ## Install from requirements-dev-lock.txt (reproducible, what CI uses)
	pip install -r requirements-dev-lock.txt
	pip install -e . --no-deps

lock: ## Regenerate requirements-lock.txt and requirements-dev-lock.txt from pyproject.toml
	@command -v pip-compile >/dev/null 2>&1 || { echo "pip-compile not found. Run: pip install pip-tools" >&2; exit 1; }
	pip-compile --quiet --resolver=backtracking --output-file=requirements-lock.txt pyproject.toml
	pip-compile --quiet --resolver=backtracking --extra=dev --extra=api --output-file=requirements-dev-lock.txt pyproject.toml
	@echo "Lock files regenerated. Commit both requirements-lock.txt and requirements-dev-lock.txt."

lock-check: ## Verify lock files are in sync with pyproject.toml (used in CI)
	@command -v pip-compile >/dev/null 2>&1 || { echo "pip-compile not found. Run: pip install pip-tools" >&2; exit 1; }
	@cp requirements-lock.txt requirements-lock.txt.bak
...
api-compatibility: ## Verify the frozen v1 Python and HTTP contracts
	python scripts/check_api_compatibility.py

release-governance: ## Verify maturity evidence and security response commitments
...
benchmark-distributed-async-contract: ## Validate the multi-node async collection contract without provisioning
	$(PYTHON_BIN) benchmarks/run_distributed_async_matrix.py \
		...
		--output-dir /tmp/stateset-distributed-async-contract --dry-run

benchmark-distributed-async-run: ## Execute multi-node async matrix (MANIFEST and OUTPUT_DIR required)
	@test -n "$(MANIFEST)" || { echo "Usage: make $@ MANIFEST=path OUTPUT_DIR=path [EXTRA_ARGS='...']" >&2; exit 2; }
	@test -n "$(OUTPUT_DIR)" || { echo "Usage: make $@ MANIFEST=path OUTPUT_DIR=path [EXTRA_ARGS='...']" >&2; exit 2; }
	$(PYTHON_BIN) benchmarks/run_distributed_async_matrix.py $(MANIFEST) \
		--output-dir $(OUTPUT_DIR) $(EXTRA_ARGS)

benchmark-distributed-async-gate: ## Validate retained multi-node async evidence (INPUTS required; optional OUTPUT)
