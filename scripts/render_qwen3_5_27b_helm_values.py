#!/usr/bin/env python
"""
Render a Helm values override for deploying Qwen3.5-27B via this repo's Helm chart.
"""
...
def build_values_from_manifest(
    ...
    *,
    ...
    max_model_len = recommended.get("max_model_len", 262144)
    trust_remote_code = recommended.get("trust_remote_code", True)
    reasoning_parser = recommended.get("reasoning_parser", "qwen3")
    tool_call_parser = recommended.get("tool_call_parser", "qwen3_coder")
    enable_auto_tool_choice = recommended.get("enable_auto_tool_choice", True)
    language_model_only = recommended.get("language_model_only", True)
    gpu_memory_utilization = recommended.get("gpu_memory_utilization", 0.90)
    ...
    if finetuned:
        ...
        if not model_path:
            model_path = "/models/qwen3-5-27b/merged"
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Render Helm values overrides from Qwen3.5-27B serving_manifest.json",
    )
    parser.add_argument(
        "--manifest",
        required=True,
        help="Path to serving_manifest.json produced by the Qwen3.5-27B finetune script.",
    )
    parser.add_argument(
        "--model-path",
        default=None,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(e.g. gs://bucket/qwen3-5-27b/runs/<run>/merged)."
        ),
    )
