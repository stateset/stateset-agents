#!/usr/bin/env python
"""
Render a Helm values override for deploying GLM 5.2 via this repo's Helm chart.
"""
...
def build_values_from_manifest(
    ...
    *,
    ...
    if finetuned:
        ...
        if not model_path:
            model_path = "/models/glm5-2/merged"
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Render Helm values overrides from GLM 5.2 serving_manifest.json",
    )
    parser.add_argument(
        "--manifest",
        required=True,
        help="Path to serving_manifest.json produced by the GLM 5.2 finetune script.",
    )
    parser.add_argument(
        "--model-path",
        default=None,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(e.g. gs://bucket/glm5-2/runs/<run>/merged)."
        ),
    )
