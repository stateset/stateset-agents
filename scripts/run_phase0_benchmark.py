...
def _build_env_reward(adapter: TaskAdapter, train_examples: list) -> tuple[Any, Any, list[dict[str, Any]]]:
...
def train_with_trainer(
    ...
    try:
        ...
        if trainer == "gspo":
            ...
            trained = asyncio.run(train_with_gspo(
                config=cfg,
                agent=trainer_agent,
                environment=env,
        ...
        if trainer == "grpo":
            ...
            trained = asyncio.run(train_with_trl_grpo(
                config=cfg,
                agent=trainer_agent,
                environment=env,
                reward_model=reward_fn,
        ...
        if trainer == "dapo":
            ...
            model, tokenizer, _metrics = asyncio.run(train_with_dapo(
                model_name=model_name,
        ...
        return None, time.time() - t0, f"trainer={trainer!r} not recognized. Use gspo, grpo, or dapo."
    except ImportError as e:
        return None, time.time() - t0, f"Missing extras: {e}. Install via `pip install -e '.[training]'`."
...
def main() -> int:
    ...
    parser.add_argument(
        "--task", choices=sorted(TASKS), default="gsm8k",
    ...
    parser.add_argument(
        "--output-dir", default="outputs/phase0_run",
    ...
    if args.smoke_test:
        ...
        logger.info("Smoke test passed — pipeline is operational for task=%s.", adapter.name)
        return 0
    ...
    if not args.skip_baseline:
        logger.info("Evaluating baseline (un-fine-tuned) on %d examples…", len(eval_examples))
        t0 = time.time()
        ...
        logger.info(
            ...
            baseline["pass_at_1"], baseline["parse_rate"],
        )
    ...
    if args.train:
        ...
        if not gpu_available:
            ...
            result["metrics"]["train_error"] = "No CUDA GPU detected. Use --smoke-test or run on a GPU host."
            logger.warning("--train was set but no GPU detected. Skipping training, baseline only.")
        else:
            logger.info("Invoking %s trainer with %d train examples…", args.trainer, len(train_examples))
            ...
            if trained_agent is None:
                ...
                t0 = time.time()
                post = _evaluate_with_agent(args.model, adapter, eval_examples, trained_agent=trained_agent)
