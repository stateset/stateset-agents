...
class FrameworkBenchmark:
            ...
            class MockAgent:
                ...
                async def generate_response(self, messages, **kwargs):
                    ...
                    self.conversation_history.append(
                        {"user": messages["content"], "assistant": response}
                    )
                    return response
    ...
    async def benchmark_training_performance(
        ...
        results = await benchmark_async_function(
            training_func, warmup_runs=1, benchmark_runs=num_episodes
        )
    ...
    async def run_full_benchmark_suite(
        ...
        try:
            agent_results = await self.benchmark_agent_response_generation(
                agent_class=agent_class, config=config
            )
        ...
        try:
            training_results = await self.benchmark_training_performance(
                trainer_class=trainer_class, num_episodes=2
            )
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of messages for agent benchmark",
    )
    parser.add_argument(
        "--num-conversations",
        type=int,
        default=5,
        help="Number of conversations for memory benchmark",
    )
    parser.add_argument(
        "--num-episodes",
        type=int,
    ...
    for benchmark_name, benchmark_results in results["benchmarks"].items():
        if "error" in benchmark_results:
            ...
            avg = benchmark_results.get("avg_duration", 0.0)
            print(f"✅ {benchmark_name}: avg duration {avg:.3f}s")
        else:
