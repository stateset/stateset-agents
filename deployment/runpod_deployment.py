...
class RunPodDeploymentManager:
    ...
    async def deploy_training_job(
        ...
        try:
            ...
            # Launch pods
            pods = await self._launch_pods(deployment_config, distributed_config.world_size)
    ...
    async def _launch_pods(self, deployment_config: Dict[str, Any], num_pods: int) -> List[Dict[str, Any]]:
        ...
        for i in range(num_pods):
            ...
            pod_config["env"]["MASTER_ADDR"] = "127.0.0.1"  # Will be updated after first pod
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                return {
                    ...
                    "name": pod_config["name"],
                    "status": "running",
                    "ip": pod_details.get("runtime", {}).get("ports", {}).get("22/tcp", [{}])[0].get("ip"),
    ...
    async def _wait_for_pod_ready(self, pod_id: str, timeout: int = 300):
        ...
        while time.time() - start_time < timeout:
            ...
            if pod.get("desiredStatus") == "RUNNING" and pod.get("runtime", {}).get("uptimeInSeconds", 0) > 0:
                logger.info(f"Pod {pod_id} is ready")
                return
            
            await asyncio.sleep(10)
        
        raise TimeoutError(f"Pod {pod_id} failed to become ready within {timeout} seconds")
    ...
    async def _log_deployment_metrics(self, deployment_id: str):
        ...
        avg_gpu_utilization = sum(
            ...
            for pod_metrics in deployment.metrics.values()
        ) / len(deployment.metrics) if deployment.metrics else 0
        
        # Log metrics
        await self.monitoring.log_metric("runpod.deployment.cost", total_cost)
        await self.monitoring.log_metric("runpod.deployment.gpu_utilization", avg_gpu_utilization)
        await self.monitoring.log_metric("runpod.deployment.num_pods", len(deployment.pods))
    
    async def scale_deployment(self, deployment_id: str, target_pods: int) -> Dict[str, Any]:
    ...
    async def _scale_up(self, deployment: DeploymentStatus, num_pods: int) -> List[Dict[str, Any]]:
        """Scale up deployment"""
    ...
    def get_cost_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cost_per_deployment": total_cost / len(self.deployments) if self.deployments else 0
        }
...
def create_code_bundle(source_dir: str, output_path: Optional[str] = None) -> str:
    ...
    if output_path is None:
        output_path = f"/tmp/grpo_bundle_{datetime.now().strftime('%Y%m%d_%H%M%S')}.tar.gz"
...
def get_runpod_config_from_env() -> RunPodConfig:
    ...
    return RunPodConfig(
        ...
        image_name=os.getenv("RUNPOD_IMAGE", "runpod/pytorch:2.1.0-py3.10-cuda11.8.0-devel"),
