...
class TRLGRPOTrainerWrapper:
    ...
    def _create_grpo_config(self) -> GRPOConfig:
        ...
        return GRPOConfig(
            ...
            report_to=self.config.report_to.split(",") if self.config.report_to != "none" else [],
...
async def train_with_trl_grpo(
    ...
    if config.report_to == "wandb" and config.wandb_project:
        wandb.init(
            ...
            name=config.run_name or f"trl-grpo-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
    ...
    if train_data:
        ...
        for episode in range(min(100, config.num_episodes)):  # Generate some initial data
