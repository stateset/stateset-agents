...
class TrajectoryData:
    ...
    def from_trajectory(cls, trajectory: Trajectory, reward: float) -> "TrajectoryData":
        ...
        return cls(
            ...
            raw_reward=reward,
            learned_reward=0.0,
            computational_cost=len(trajectory.get_prompt()) + len(trajectory.get_last_response()),
...
class NeuralRewardModel(nn.Module):
    ...
    def __init__(
        ...
        # Input projection
        self.input_proj = nn.Linear(embedding_dim * 2, hidden_dim)  # concat prompt + response
...
class NeuralRewardTrainer:
    ...
    def train(
        ...
        for epoch in range(self.max_epochs):
            ...
            # Monitor metrics
            if self.monitoring:
                asyncio.create_task(self.monitoring.log_metric(
                    "neural_reward.train_loss", train_loss,
                    tags={"epoch": epoch + 1}
                ))
                if val_loss is not None:
                    asyncio.create_task(self.monitoring.log_metric(
                        "neural_reward.val_loss", val_loss,
                        tags={"epoch": epoch + 1}
                    ))
...
class NeuralRewardFunction(RewardFunction):
    ...
    async def compute_reward(
        ...
        try:
            ...
            if (self.evaluation_count % self.update_frequency == 0 and 
                len(self.trainer.replay_buffer) >= self.min_trajectories):
    ...
    async def _update_model(self):
        ...
        try:
            ...
            logger.info(f"Neural reward model updated. Train loss: {results.get('final_train_loss', 'N/A')}")
