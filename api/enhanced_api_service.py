...
from ..utils.security import InputValidator, AuthService, SecurityMonitor
...
class AgentConfigRequest(BaseModel):
    ...
    max_new_tokens: int = Field(512, description="Maximum tokens to generate", ge=1, le=4096)
...
class ConversationRequest(BaseModel):
    ...
    max_tokens: int = Field(512, description="Maximum tokens in response", ge=1, le=4096)
...
class TrainingRequest(BaseModel):
    ...
    environment_scenarios: List[Dict[str, Any]] = Field(..., description="Training scenarios")
    reward_config: Dict[str, Any] = Field(..., description="Reward function configuration")
    num_episodes: int = Field(100, description="Number of training episodes", ge=1, le=10000)
...
class MetricsResponse(BaseModel):
    ...
    system_metrics: Dict[str, Union[int, float]] = Field(..., description="System metrics")
...
class TrainingService:
    ...
    async def start_training(self, request: TrainingRequest) -> str:
        ...
        reward_fn = CompositeReward([
            HelpfulnessReward(weight=request.reward_config.get("helpfulness_weight", 0.7)),
        ...
        asyncio.create_task(self._run_training(training_id, agent, environment, reward_fn, request))
        
        return training_id
    
    async def _run_training(self, training_id: str, agent: MultiTurnAgent, 
                           environment, reward_fn, request: TrainingRequest):
        """Run training job."""
        try:
