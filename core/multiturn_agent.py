...
class MultiTurnAgent(Agent):
    ...
    async def _customer_service_strategy(self, context: ConversationContext) -> str:
        ...
        recent_user_messages = [
            turn["content"] for turn in context.history 
    ...
    async def _technical_support_strategy(self, context: ConversationContext) -> str:
        ...
        last_user_message = context.history[-1].get("content", "") if context.history else ""
    ...
    def _build_technical_support_prompt(self, context: ConversationContext) -> str:
        ...
        if technical_terms:
            prompt_parts.append(f"Technical terms mentioned: {', '.join(technical_terms)}")
            prompt_parts.append("")
    ...
    def _build_educational_prompt(self, context: ConversationContext, user_level: str) -> str:
    ...
    async def _analyze_user_input(self, context: ConversationContext, user_message: str):
        """Analyze user input for intent and entities"""
        # Simple intent detection
        if any(word in user_message.lower() for word in ["help", "problem", "issue", "error"]):
            context.intent = "support"
        elif any(word in user_message.lower() for word in ["buy", "purchase", "price", "cost"]):
            context.intent = "sales"
        elif any(word in user_message.lower() for word in ["learn", "how", "what", "explain"]):
    ...
    def _extract_technical_terms(self, text: str) -> List[str]:
        ...
        technical_keywords = [
            "api", "database", "server", "client", "http", "ssl", "tcp", "ip",
            "json", "xml", "rest", "soap", "auth", "token", "session",
            "cache", "queue", "load", "performance", "latency", "throughput"
        ]
    ...
    async def _compress_context(self, context: ConversationContext):
        ...
        if middle_turns:
            ...
            context.history = (
                ...
                [{"role": "system", "content": f"[Summary of previous turns: {summary}]"}] +
    ...
    def _create_conversation_summary(self, turns: List[Dict[str, Any]]) -> str:
        ...
        assistant_messages = [turn["content"] for turn in turns if turn.get("role") == "assistant"]
    ...
    async def compute_multiturn_rewards(
        ...
        for reward_func in reward_functions:
            try:
                result = await reward_func.compute_reward(turns, context.get_context_summary())
                total_reward += result.score * reward_func.weight
                reward_breakdown[reward_func.__class__.__name__] = result.score
            except Exception as e:
                logger.error(f"Reward function {reward_func.__class__.__name__} failed: {e}")
        ...
        reward_breakdown.update({
            ...
            "goal_achievement": self._evaluate_goal_achievement(context, ground_truth)
        })
