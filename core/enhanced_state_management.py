...
class InMemoryCacheBackend(CacheBackend):
    ...
    async def _evict(self):
        ...
        if self.eviction_policy == EvictionPolicy.LRU:
            ...
            least_frequent_key = min(self.access_frequency.keys(), key=lambda k: self.access_frequency[k])
...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379", key_prefix: str = "grpo:"):
    ...
    async def cleanup_expired_conversations(self) -> int:
        ...
        for key in list(self.state_manager.state.keys()):
            if key.startswith(self.conversation_prefix):
                ...
                if conversation and current_time - conversation["last_activity"] > self.session_timeout:
    ...
    async def _periodic_cleanup(self):
        ...
        while True:
            try:
                ...
                cleaned = await self.conversation_manager.cleanup_expired_conversations()
                if cleaned > 0:
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        await state_service.state_manager.set("user:123", {"name": "Alice", "preferences": {"theme": "dark"}})
        ...
        conversation = await state_service.conversation_manager.get_conversation(conversation_id)
