...
from typing import (
    ...
    TypeVar, Generic, AsyncContextManager, Set
)
...
class PooledResource(Generic[T]):
    ...
    def is_idle_too_long(self, max_idle: float) -> bool:
        ...
        return (self.info.state == ResourceState.IDLE and 
                time.time() - self.info.last_used > max_idle)
...
class AsyncResourcePool(Generic[T]):
    ...
    async def acquire(self, timeout: Optional[float] = None) -> AsyncContextManager[T]:
        ...
        try:
            ...
            try:
                ...
                async with self._lock:
                    if len(self._pool) < self.max_size:
                        ...
                        raise RuntimeError(f"Pool {self.name} exhausted (timeout after {timeout}s)")
            
            # Validate resource
            if not await self.factory.validate_resource(pooled_resource.resource):
                logger.warning(f"Resource {pooled_resource.info.resource_id} failed validation")
            ...
            try:
                ...
                async with self._lock:
                    ...
                    if self._stats.average_response_time == 0:
                        ...
                        self._stats.average_response_time = (
                            (self._stats.average_response_time * (self._stats.successful_requests - 1) + response_time) /
                ...
                logger.error(f"Error using resource {pooled_resource.info.resource_id}: {e}")
                raise
    ...
    async def _perform_health_check(self) -> None:
        ...
        async with self._lock:
            for resource in self._pool:
                ...
                if (resource.is_idle_too_long(self.max_idle_time) and 
                ...
                if resource.info.state == ResourceState.IDLE:
                    try:
                        is_valid = await self.factory.validate_resource(resource.resource)
                        if not is_valid:
                            logger.warning(f"Resource {resource.info.resource_id} failed health check")
                            resources_to_remove.append(resource)
                    except Exception as e:
                        logger.error(f"Health check error for {resource.info.resource_id}: {e}")
                        resources_to_remove.append(resource)
    ...
    async def scale(self, target_size: int) -> None:
        ...
        if target_size < self.min_size or target_size > self.max_size:
            raise ValueError(f"Target size {target_size} outside valid range [{self.min_size}, {self.max_size}]")
        ...
        async with self._lock:
            ...
            if target_size > current_size:
                ...
                resources_to_remove = []
                for resource in self._pool:
                    if (resource.info.state == ResourceState.IDLE and 
                        len(self._pool) - len(resources_to_remove) > target_size):
                        resources_to_remove.append(resource)
...
class AsyncTaskManager:
    ...
    async def start(self) -> None:
        ...
        logger.info(f"Task manager started with max {self.max_concurrent_tasks} concurrent tasks")
