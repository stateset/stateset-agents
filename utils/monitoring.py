    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        if HAS_PSUTIL:
            summary["system_metrics"] = {
                "cpu_usage": {
                    "current": list(self.system_metrics["cpu_usage"])[-1] if self.system_metrics["cpu_usage"] else 0,
                    "average": statistics.mean(self.system_metrics["cpu_usage"]) if self.system_metrics["cpu_usage"] else 0,
                    "max": max(self.system_metrics["cpu_usage"]) if self.system_metrics["cpu_usage"] else 0
                },
                "memory_usage": {
                    "current": list(self.system_metrics["memory_usage"])[-1] if self.system_metrics["memory_usage"] else 0,
                    "average": statistics.mean(self.system_metrics["memory_usage"]) if self.system_metrics["memory_usage"] else 0,
                    "max": max(self.system_metrics["memory_usage"]) if self.system_metrics["memory_usage"] else 0
                },
                "disk_usage": {
                    "current": list(self.system_metrics["disk_usage"])[-1] if self.system_metrics["disk_usage"] else 0,
                    "average": statistics.mean(self.system_metrics["disk_usage"]) if self.system_metrics["disk_usage"] else 0,
                    "max": max(self.system_metrics["disk_usage"]) if self.system_metrics["disk_usage"] else 0
                }
            }
        ...
        summary["application_metrics"] = {
            ...
            "requests_per_second": list(self.app_metrics["requests_per_second"])[-1] if self.app_metrics["requests_per_second"] else 0,
            "average_response_time": statistics.mean(self.app_metrics["response_times"]) if self.app_metrics["response_times"] else 0,
            "active_conversations": self.app_metrics["active_conversations"],
            "training_iterations": self.app_metrics["training_iterations"],
            "average_reward_score": statistics.mean(self.app_metrics["reward_scores"]) if self.app_metrics["reward_scores"] else 0,
...
class HealthChecker:
    ...
    async def run_all_checks(self) -> Dict[str, Any]:
        ...
        overall_status = "healthy" if all_healthy else ("critical" if critical_failing else "degraded")
        ...
        return {
            ...
            "summary": {
                ...
                "critical_failing": len([r for r in results if r["status"] != "healthy" and r["critical"]])
            }
        }
...
class MonitoringService:
    ...
    def _setup_default_alert_rules(self):
        ...
        def high_cpu_condition(metrics):
            cpu_usage = metrics.get("system_metrics", {}).get("cpu_usage", {}).get("current", 0)
            return cpu_usage > 80
        ...
        def high_memory_condition(metrics):
            memory_usage = metrics.get("system_metrics", {}).get("memory_usage", {}).get("current", 0)
