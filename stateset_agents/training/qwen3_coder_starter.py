...
_FAMILY_LABEL = "Qwen3 Coder"
_DISPLAY_NAME = "Qwen3 Coder"
...
QWEN3_CODER_DEFAULT_OUTPUT_DIR = "./outputs/qwen3_coder_gspo"
# Attention-only LoRA targets: the 128-expert MoE MLPs are impractical LoRA
# targets (one adapter pair per expert per layer), so only the attention
...
QWEN3_CODER_CONFIG_SUFFIXES = {".json", ".js", ".yaml", ".yml"}
...
def get_qwen3_coder_system_prompt(task: str = "customer_service") -> str:
    """Return a task-specific system prompt for Qwen3 Coder."""
...
def get_qwen3_coder_profile_overrides(
    ...
    return _common.select_profile_overrides(
        ...
        choices=QWEN3_CODER_STARTER_PROFILE_CHOICES,
        family_label=_FAMILY_LABEL,
    )


def get_qwen3_coder_profile_description(starter_profile: str = "balanced") -> str:
    """Return the human-readable description for a starter profile."""
    return _common.select_profile_description(
        starter_profile,
        descriptions=QWEN3_CODER_STARTER_PROFILE_DESCRIPTIONS,
        choices=QWEN3_CODER_STARTER_PROFILE_CHOICES,
        family_label=_FAMILY_LABEL,
    )


def summarize_qwen3_coder_config(config: Qwen3CoderConfig) -> dict[str, Any]:
    """Summarize the most relevant first-run properties for a resolved config."""
    return _common.summarize_config(config)


def describe_qwen3_coder_starter_profiles(
    task: str = "customer_service",
    model_name: str = QWEN3_CODER_BASE_MODEL,
) -> dict[str, Any]:
    ...
    return _common.describe_starter_profiles(
        ...
        choices=QWEN3_CODER_STARTER_PROFILE_CHOICES,
        get_config=get_qwen3_coder_config,
        get_description=get_qwen3_coder_profile_description,
        summarize=summarize_qwen3_coder_config,
    )


@dataclass
class Qwen3CoderConfig(_common.StarterConfigMixin):
    """Lightweight configuration container for Qwen3 Coder post-training."""
    ...
    lora_target_modules: list[str] = field(
        default_factory=lambda: list(QWEN3_CODER_LORA_TARGET_MODULES)
    )
    ...
    output_dir: str = QWEN3_CODER_DEFAULT_OUTPUT_DIR
    ...
    _system_prompt = staticmethod(get_qwen3_coder_system_prompt)
    _wandb_base_tags = ("qwen3-coder", "gspo")
    _wandb_project_default = "qwen3_coder-gspo"

    def validate(self) -> list[str]:
        return validate_qwen3_coder_config(self)


def get_qwen3_coder_config(
    model_name: str = QWEN3_CODER_BASE_MODEL,
    task: str = "customer_service",
    starter_profile: str = "balanced",
    ...
    **overrides: Any,
    ...
    """Create a tuned first-run Qwen3 Coder configuration."""
...
def create_qwen3_coder_agent_config(config: Qwen3CoderConfig) -> AgentConfig:
    """Create the matching AgentConfig for Qwen3 Coder."""
    return _common.create_agent_config(config)


def get_qwen3_coder_gspo_overrides(config: Qwen3CoderConfig) -> dict[str, Any]:
    """Return the GSPO override payload for Qwen3 Coder."""
    return _common.build_gspo_overrides(config)


def get_qwen3_coder_gspo_config(
    config: Qwen3CoderConfig,
    base_config: TrainingConfig | None = None,
):
    """Create the GSPOConfig used for Qwen3 Coder post-training."""
    return _common.build_gspo_config(
        config, base_config, get_config_for_task, get_qwen3_coder_gspo_overrides
    )


def validate_qwen3_coder_config(config: Qwen3CoderConfig) -> list[str]:
...
def create_qwen3_coder_preview(
    config: Qwen3CoderConfig,
    ...
    return _common.create_preview(
        config,
        warnings,
        agent_config_fn=create_qwen3_coder_agent_config,
        summarize_fn=summarize_qwen3_coder_config,
        gspo_overrides_fn=get_qwen3_coder_gspo_overrides,
    )


def load_qwen3_coder_config_file(path: str | Path) -> Qwen3CoderConfig:
    """Load a Qwen3 Coder starter config from JSON or YAML."""
    return _common.load_config_file(
        path,
        config_cls=Qwen3CoderConfig,
        suffixes=QWEN3_CODER_CONFIG_SUFFIXES,
        family_label=_FAMILY_LABEL,
...
def write_qwen3_coder_config_file(
    config: Qwen3CoderConfig,
    path: str | Path,
    include_preview: bool = False,
) -> Path:
    """Write a Qwen3 Coder starter config to JSON or YAML."""
    return _common.write_config_file(
        config,
        path,
        include_preview,
        preview_fn=create_qwen3_coder_preview,
        suffixes=QWEN3_CODER_CONFIG_SUFFIXES,
        family_label=_FAMILY_LABEL,
        display_name=_DISPLAY_NAME,
        logger=logger,
...
async def run_qwen3_coder_config(
    config: Qwen3CoderConfig,
    dry_run: bool = False,
) -> Any:
    """Run or preview a Qwen3 Coder GSPO job from a resolved config object."""
    return await _common.run_starter_config(
        config,
        dry_run,
        preview_fn=create_qwen3_coder_preview,
        gspo_config_fn=get_qwen3_coder_gspo_config,
        agent_config_fn=create_qwen3_coder_agent_config,
        display_name=_DISPLAY_NAME,
        logger=logger,
    )
...
async def finetune_qwen3_coder(
    model_name: str = QWEN3_CODER_BASE_MODEL,
    task: str = "customer_service",
    starter_profile: str = "balanced",
    use_lora: bool | None = None,
    ...
    """Run or preview a first GSPO post-training job for Qwen3 Coder."""
    return await _common.finetune_starter(
        get_config_fn=get_qwen3_coder_config,
        run_fn=run_qwen3_coder_config,
        model_name=model_name,
        task=task,
        starter_profile=starter_profile,
        use_lora=use_lora,
