...
def _read_mapping_file(path: Path) -> dict[str, Any]:
    ...
    if suffix not in KIMI_K3_CONFIG_SUFFIXES:
...
def get_kimi_k3_profile_overrides(starter_profile: str = "balanced") -> dict[str, Any]:
    ...
    if starter_profile not in profiles:
        supported = ", ".join(KIMI_K3_STARTER_PROFILE_CHOICES)
...
def get_kimi_k3_profile_description(starter_profile: str = "balanced") -> str:
    ...
    if starter_profile not in KIMI_K3_STARTER_PROFILE_DESCRIPTIONS:
        supported = ", ".join(KIMI_K3_STARTER_PROFILE_CHOICES)
        raise ValueError(f"Unsupported Kimi starter profile: {starter_profile}. Use one of: {supported}.")
    return KIMI_K3_STARTER_PROFILE_DESCRIPTIONS[starter_profile]
...
def describe_kimi_k3_starter_profiles(
    ...
    for starter_profile in KIMI_K3_STARTER_PROFILE_CHOICES:
        ...
        profiles[starter_profile] = {
            "description": get_kimi_k3_profile_description(starter_profile),
    ...
    return {
        "model_name": model_name,
        "task": task,
        "default_profile": KIMI_K3_STARTER_PROFILE_CHOICES[0],
        "profiles": profiles,
    }
...
class KimiK3Config:
    ...
    def __post_init__(self) -> None:
        if self.system_prompt is None:
            self.system_prompt = get_kimi_k3_system_prompt(self.task)
        ...
        if self.use_wandb:
            ...
            if self.wandb_project is None:
                self.wandb_project = "kimi_k3-gspo"
        else:
            self.report_to = "none"
    ...
    @classmethod
    def from_dict(cls, config_dict: dict[str, Any]) -> KimiK3Config:
...
def get_kimi_k3_config(
    ...
    **overrides: Any,
    ...
    resolved_overrides = get_kimi_k3_profile_overrides(starter_profile)
    ...
    logger.info(
        "Created Kimi-K3 config for task=%s profile=%s model=%s",
        config.task,
...
def get_kimi_k3_gspo_config(
    ...
    return GSPOConfig.from_training_config(
        resolved_base,
        **get_kimi_k3_gspo_overrides(config),
    )
...
def create_kimi_k3_preview(
    ...
    return {
        ...
        "gspo_overrides": get_kimi_k3_gspo_overrides(config),
...
def load_kimi_k3_config_file(path: str | Path) -> KimiK3Config:
    ...
    loaded = KimiK3Config.from_dict(config_payload)
    logger.info("Loaded Kimi-K3 config from %s", config_path)
    return loaded
...
def write_kimi_k3_config_file(
    ...
    payload = create_kimi_k3_preview(config) if include_preview else config.to_dict()
    written_path = _write_mapping_file(payload, Path(path))
    logger.info("Wrote Kimi-K3 config to %s", written_path)
    return written_path
...
async def run_kimi_k3_config(
    ...
    if dry_run:
        return create_kimi_k3_preview(config, warnings=warnings)
    ...
    logger.info("Initializing Kimi-K3 agent")
...
async def finetune_kimi_k3(
    ...
    return await run_kimi_k3_config(config, dry_run=dry_run)
