...
def _read_mapping_file(path: Path) -> dict[str, Any]:
    ...
    if suffix not in GLM5_2_CONFIG_SUFFIXES:
...
def get_glm5_2_profile_overrides(starter_profile: str = "balanced") -> dict[str, Any]:
    ...
    if starter_profile not in profiles:
        supported = ", ".join(GLM5_2_STARTER_PROFILE_CHOICES)
        raise ValueError(
...
def get_glm5_2_profile_description(starter_profile: str = "balanced") -> str:
    ...
    if starter_profile not in GLM5_2_STARTER_PROFILE_DESCRIPTIONS:
        supported = ", ".join(GLM5_2_STARTER_PROFILE_CHOICES)
        raise ValueError(
            f"Unsupported GLM starter profile: {starter_profile}. Use one of: {supported}."
        )
    return GLM5_2_STARTER_PROFILE_DESCRIPTIONS[starter_profile]
...
def describe_glm5_2_starter_profiles(
    ...
    for starter_profile in GLM5_2_STARTER_PROFILE_CHOICES:
        ...
        profiles[starter_profile] = {
            "description": get_glm5_2_profile_description(starter_profile),
    ...
    return {
        "model_name": model_name,
        "task": task,
        "default_profile": GLM5_2_STARTER_PROFILE_CHOICES[0],
        "profiles": profiles,
    }
...
class Glm52Config:
    ...
    def __post_init__(self) -> None:
        if self.system_prompt is None:
            self.system_prompt = get_glm5_2_system_prompt(self.task)
        ...
        if self.use_wandb:
            ...
            if self.wandb_project is None:
                self.wandb_project = "glm5_2-gspo"
        else:
            self.report_to = "none"
    ...
    @classmethod
    def from_dict(cls, config_dict: dict[str, Any]) -> Glm52Config:
...
def get_glm5_2_config(
    ...
    **overrides: Any,
    ...
    resolved_overrides = get_glm5_2_profile_overrides(starter_profile)
    ...
    logger.info(
        "Created GLM 5.2 config for task=%s profile=%s model=%s",
        config.task,
...
def get_glm5_2_gspo_config(
    ...
    return GSPOConfig.from_training_config(
        resolved_base,
        **get_glm5_2_gspo_overrides(config),
    )
...
def create_glm5_2_preview(
    ...
    return {
        ...
        "gspo_overrides": get_glm5_2_gspo_overrides(config),
...
def load_glm5_2_config_file(path: str | Path) -> Glm52Config:
    ...
    loaded = Glm52Config.from_dict(config_payload)
    logger.info("Loaded GLM 5.2 config from %s", config_path)
    return loaded
...
def write_glm5_2_config_file(
    ...
    payload = create_glm5_2_preview(config) if include_preview else config.to_dict()
...
async def run_glm5_2_config(
    ...
    if dry_run:
        return create_glm5_2_preview(config, warnings=warnings)
    ...
    logger.info("Initializing GLM 5.2 agent")
...
async def finetune_glm5_2(
    ...
    return await run_glm5_2_config(config, dry_run=dry_run)
