...
class MultimodalConsistencyReward(RewardComponent):
    ...
    async def compute(
        ...
        **kwargs,
        ...
        has_ack = any(
            phrase.lower() in response.lower() for phrase in visual_ack_phrases
        )
        ...
        visual_count = sum(
            1 for ref in visual_ref_clusters if ref.lower() in response.lower()
        )
...
class CodeExecutionReward(RewardComponent):
    ...
    async def compute(
        ...
        **kwargs,
        ...
        quality_score = sum(
            1 for indicator in quality_indicators if indicator in response
    ...
    async def compute(
        ...
        **kwargs,
        ...
        has_earlier_reference = any(
            indicator.lower() in response.lower() for indicator in reference_indicators
        )
        ...
        for earlier_msg in earlier_messages[-3:]:  # Check last 3 messages
            if earlier_msg.lower() in response.lower():
