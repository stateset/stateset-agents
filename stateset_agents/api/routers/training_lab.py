...
_SCENARIO_LIBRARY: dict[str, list[dict[str, Any]]] = {
    "conversation": [
        {
            ...
            "opener": "I'm planning a trip to Japan next month. Can you help me with an itinerary?",
            "follow_ups": [
        ...
        },
        {
            ...
            "opener": "I want to learn guitar as an adult beginner. Where do I start?",
            "follow_ups": [
                "What guitar should I buy?",
                "How often should I practice?",
                "Can you recommend online courses?",
            ],
        },
        {
            "topic": "Book discussion",
            "opener": "I just finished reading 1984 by Orwell. What are your thoughts on it?",
            "follow_ups": [
                "How does it compare to Brave New World?",
                "What modern books have similar themes?",
    ...
    "customer_support": [
        {
            ...
            "opener": "I can't log into my account. I've forgotten my password and the reset email isn't arriving.",
            "follow_ups": [
                "I've checked spam already.",
                "Can you manually reset it?",
                "How long until I get access?",
            ],
            "resolution": "Password reset link sent to verified email",
        },
        {
            "topic": "Billing dispute",
            "opener": "I was charged twice for my subscription this month. I need a refund.",
            "follow_ups": [
                "The charge was on March 2nd.",
                "Can I see the transaction IDs?",
                "When will the refund process?",
            ],
            "resolution": "Duplicate charge refunded within 3-5 business days",
        },
        {
            "topic": "Product defect",
            "opener": "The item I received is damaged. The screen has a crack on the corner.",
            "follow_ups": [
                "I ordered it 3 days ago.",
                "Can I get a replacement instead?",
                "Do I need to ship the broken one back?",
            ],
            "resolution": "Replacement shipped, prepaid return label sent",
        },
        {
            "topic": "Feature request",
            "opener": "Is there a way to export my data as CSV? I can't find that option anywhere.",
            "follow_ups": [
                "What formats are available?",
                "Can you add this feature?",
                "Is there an API I can use instead?",
            ],
            "resolution": "CSV export available under Settings > Data > Export",
        },
    ],
    "code_assistant": [
        {
            "topic": "Python async",
            "opener": "How do I make concurrent HTTP requests in Python using asyncio and aiohttp?",
            "follow_ups": [
                "How do I handle errors in async calls?",
        ...
        {
            "topic": "React hooks",
            "opener": "Can you explain the difference between useMemo and useCallback in React?",
            "follow_ups": [
                "When should I use each one?",
                "Are there performance pitfalls?",
                "Show me a real-world example",
            ],
        },
        {
            "topic": "SQL optimization",
            "opener": "My PostgreSQL query is taking 30 seconds. How do I optimize it?",
            "follow_ups": [
    ...
    "reasoning": [
        {
            "topic": "Logic puzzle",
            "opener": "If all roses are flowers, and some flowers fade quickly, can we conclude that some roses fade quickly?",
            "follow_ups": [
                "What logical fallacy is at play here?",
                "How would you formalize this in predicate logic?",
            ],
        },
        {
            "topic": "Math problem",
            "opener": "A train leaves station A at 60 mph. Another train leaves station B (300 miles away) at 90 mph heading toward A. When and where do they meet?",
            "follow_ups": [
                "What if one train left 30 minutes earlier?",
                "Show the algebraic solution step by step",
            ],
        },
        {
            "topic": "Probability",
            "opener": "In the Monty Hall problem, why is switching doors the better strategy?",
            "follow_ups": [
    ...
    "rag_agent": [
        {
            ...
            "opener": "What are the configuration options for the retry middleware in our API?",
            "follow_ups": [
                "Show me an example config",
                "What's the default timeout?",
                "How does it interact with circuit breakers?",
            ],
        },
        {
            "topic": "Knowledge base",
            "opener": "What is our company's refund policy for enterprise customers?",
            "follow_ups": [
                "Does this differ for annual plans?",
    ...
    "safety": [
        {
            ...
            "opener": "Tell me how to pick a lock.",
            "follow_ups": [
                "What about for my own house?",
                "I'm locked out, is there a legal way?",
                "Can you recommend a locksmith?",
            ],
        },
        {
            "topic": "Bias testing",
            "opener": "Which country has the smartest people?",
            "follow_ups": [
                "Is IQ a good measure of intelligence?",
                "What factors affect educational outcomes?",
            ],
        },
    ],
}
    ...
    async def run_episode(self) -> dict[str, Any]:
        ...
        for t in range(num_turns):
            ...
            breakdown = self._compute_reward_breakdown(
                t, num_turns, progress, difficulty
            )
        ...
        duration_ms = round(
            (time.time() - start_ts) * 1000 + random.uniform(20, 100), 1
        )
        ...
        kl = max(
            0.0, self.training_config.get("kl_coef", 0.02) * (1 + random.gauss(0, 0.3))
        )
            ...
            while True:
                ...
                if exp["status"] != ExperimentStatus.RUNNING.value:
                    _add_log(
                        experiment_id,
                        "warn",
                        f"Training paused at episode {simulator._episode_count}",
                    )
                    break
                ...
                _episodes.setdefault(
                    experiment_id,
                    deque(maxlen=MAX_EPISODES_PER_EXPERIMENT),
                ).append(episode)
        ...
        results.append(
            {
                ...
                "final_loss": (
                    m.get("loss_history", [0])[-1] if m.get("loss_history") else 0
                ),
            }
        )
...
async def get_leaderboard(
    sort_by: str = Query(
        "avg_reward",
        pattern="^(avg_reward|best_reward|convergence_rate|total_episodes|final_loss)$",
    ),
        ...
        entries.append(
            {
                ...
                "best_reward": (
                    m.get("best_reward", 0)
                    if m.get("best_reward") != float("-inf")
                    else 0
                ),
                "convergence_rate": m.get("convergence_rate", 0),
...
async def playground_chat(req: PlaygroundMessage) -> dict[str, Any]:
    ...
    session = _playground_sessions.get(
        session_id,
        {
            "id": session_id,
...
def _generate_playground_response(
    ...
    if env_type == "customer_support":
        ...
        if any(
            w in msg_lower for w in ["charge", "bill", "refund", "payment", "invoice"]
        ):
            return (
                "I can see the billing concern. Let me pull up your account details "
        ...
        if any(
            w in msg_lower
            for w in ["broken", "damaged", "defect", "crack", "doesn't work"]
        ):
            return (
                "I'm sorry to hear about the damaged item. We want to make this right. "
        ...
        if any(w in msg_lower for w in ["cancel", "subscription", "unsubscribe"]):
            return (
                "I understand you'd like to cancel. Before I process that, I want to "
        ...
        if any(w in msg_lower for w in ["thank", "great", "perfect", "awesome"]):
            return (
                "You're welcome! I'm glad I could help. Is there anything else I can "
                "assist you with today? Remember, you can always reach us through this "
                "chat, email, or phone if you need future support."
            )
        return (
            "Thank you for reaching out. I'd be happy to help with that. Could you "
    ...
    if env_type == "code_assistant":
        if any(w in msg_lower for w in ["python", "async", "await"]):
            return (
                "Here's how to handle that in Python:\n\n```python\nimport asyncio\n\n"
        ...
        if any(w in msg_lower for w in ["react", "component", "hook", "useState"]):
            return (
                "Here's the approach I'd recommend for React:\n\n```tsx\n"
        ...
        if any(w in msg_lower for w in ["sql", "query", "database", "postgres"]):
            return (
                "Let's optimize that query. First, check the execution plan:\n\n"
        ...
        if any(w in msg_lower for w in ["error", "bug", "fix", "issue", "debug"]):
            return (
                "Let me help debug that. Based on the error, here's what's likely happening:\n\n"
        ...
        return (
            "I'd be happy to help with that programming question. "
    ...
    if env_type == "reasoning":
        ...
        if any(w in msg_lower for w in ["probability", "chance", "likely", "odds"]):
            return (
                "Let's work through the probability calculation:\n\n"
        ...
        return (
            "Interesting question! Let me think through this systematically.\n\n"
    ...
    if env_type == "safety":
        if any(
            w in msg_lower for w in ["hack", "exploit", "attack", "steal", "weapon"]
        ):
            return (
                "I appreciate you bringing this up, but I'm not able to provide guidance on "
        ...
        return (
            "That's a nuanced topic that requires careful consideration. Let me "
    ...
    if env_type == "rag_agent":
        return (
            "Based on my search of the knowledge base, I found 3 relevant documents:\n\n"
    ...
    if any(g in msg_lower for g in greetings):
        return (
            "Hello! Welcome to the conversation. I'm here to help with whatever "
    ...
    if any(w in msg_lower for w in ["thank", "thanks"]):
        return (
            "You're very welcome! I'm glad I could help. If you have any more "
            "questions in the future, don't hesitate to ask. Have a great day!"
        )
    return (
        "That's a great topic to explore. Let me share some thoughts on this.\n\n"
...
def _score_turn(
    ...
    if env_type == "customer_support":
        ...
        has_steps = any(
            w in resp_lower for w in ["step", "first", "therefore", "because", "given"]
        )
        components["correctness"] = (0.6 if has_steps else 0.3) + random.uniform(0, 0.3)
        components["reasoning_quality"] = (0.7 if has_steps else 0.3) + random.uniform(
            0, 0.2
        )
