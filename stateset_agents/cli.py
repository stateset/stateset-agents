...
# ``evaluate`` and ``init`` live in sibling modules (see the note on the
# subcommand-module block near the bottom of this file). They are imported
# *here*, at the position their definitions used to occupy, so that the
# `--help` command listing keeps its original order.
from stateset_agents import cli_evaluate  # noqa: E402, F401 — evaluate
from stateset_agents import cli_init  # noqa: E402, F401 — init scaffolding wizard
