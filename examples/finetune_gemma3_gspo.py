"""
Fine-tune Gemma 3 (Gemma 2) Models with GSPO

This script demonstrates how to fine-tune Google's Gemma 3 (Gemma 2) models using GSPO,
providing stable and efficient training for conversational AI tasks.
...
Usage:
    # Small model (2B) - Great for testing
    python examples/finetune_gemma3_gspo.py --model google/gemma-2-2b-it --task customer_service

    # Medium model (9B) - Balanced performance
    python examples/finetune_gemma3_gspo.py --model google/gemma-2-9b-it --task technical_support --use-lora

    # Large model (27B) - High quality
    python examples/finetune_gemma3_gspo.py --model google/gemma-2-27b-it --task sales --use-lora --use-4bit
"""
...
def get_gemma3_config(
    ...
    """
    Get optimized GSPO configuration for Gemma 3 models.
    ...
    if "2b" in model_name.lower() or "2B" in model_name:
        # Small model config (2B)
    ...
    elif "9b" in model_name.lower() or "9B" in model_name:
        # Medium model config (9B)
        config = GSPOConfig.from_training_config(
            base_config,
            num_generations=6,
            ...
            use_lora=True,  # Always use LoRA for 9B
            lora_r=64,
            lora_alpha=128,
            lora_dropout=0.05,
...
async def finetune_gemma3(
    ...
    Args:
        model_name: Gemma 3 model name (e.g., "google/gemma-2-9b-it")
    ...
    system_prompts = {
        "customer_service": "You are a helpful and empathetic customer service representative. You assist customers with their inquiries professionally and efficiently.",
        "technical_support": "You are a knowledgeable technical support specialist. You help users troubleshoot technical issues with clear, detailed explanations.",
        "sales": "You are a friendly and persuasive sales representative. You help customers discover products that meet their needs.",
    }

    # Create agent
    logger.info("Initializing Gemma 3 agent...")
    ...
    if use_wandb:
        ...
        gspo_config.wandb_project = wandb_project or f"gemma3-gspo-{task}"
        gspo_config.wandb_tags = ["gemma3", "gspo", task, model_name.split("/")[-1]]
    ...
    logger.info("\n" + "=" * 80)
    logger.info("Testing trained Gemma 3 model...")
    logger.info("=" * 80)
    ...
    logger.info(f"Gemma 3: {response}\n")
    logger.info("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune Gemma 3 models with GSPO",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Small model for testing
  python examples/finetune_gemma3_gspo.py --model google/gemma-2-2b-it

  # 9B model with LoRA
  python examples/finetune_gemma3_gspo.py --model google/gemma-2-9b-it --use-lora

  # 27B model with 4-bit quantization
  python examples/finetune_gemma3_gspo.py --model google/gemma-2-27b-it --use-4bit

  # Enable W&B logging
  python examples/finetune_gemma3_gspo.py --model google/gemma-2-9b-it --wandb --wandb-project my-project
        """,
    )
    ...
    parser.add_argument(
        ...
        help="Gemma 3 model name (e.g., google/gemma-2-9b-it)",
    )
    parser.add_argument(
        "--task",
        type=str,
