    ...
    async def compute_reward(
        ...
        for i, turn in enumerate(assistant_turns):
            ...
            if self.optimal_min <= length <= self.optimal_max:
                ...
                score = 0.5 + 0.5 * (length - self.min_length) / (
                    self.optimal_min - self.min_length
                )
            elif self.optimal_max < length <= self.max_length:
                # Linear decrease from optimal to max
                score = 0.5 + 0.5 * (self.max_length - length) / (
                    self.max_length - self.optimal_max
                )
        ...
        return RewardResult(
            ...
            metadata={
                ...
                "avg_length": sum(len(t.content) for t in assistant_turns)
                / len(assistant_turns),
            },
        )
...
class QuestionAnsweringReward(RewardFunction):
    ...
    async def compute_reward(
        ...
        if question_count == 0:
            return RewardResult(
                score=0.5, breakdown={}, metadata={"questions_found": 0}
            )
...
class SentimentReward(RewardFunction):
    ...
    async def compute_reward(
        ...
        return RewardResult(
            ...
            metadata={
                "num_turns": len(assistant_turns),
                "model": "transformer" if self.use_transformer else "textblob",
            },
        )
...
class CurriculumReward(RewardFunction):
    ...
    def __init__(
        self,
        weight: float = 1.0,
        initial_rewards: list[RewardFunction] | None = None,
        advanced_rewards: list[RewardFunction] | None = None,
...
async def test_reward_function(
    reward_fn: RewardFunction, test_conversations: list[list[ConversationTurn]]
):
...
async def main():
    ...
    parser.add_argument(
        "--list-rewards", action="store_true", help="List all available reward types"
    )
