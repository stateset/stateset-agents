"""Dedicated GLM 5.2 GSPO starter script.

Fine-tune GLM 5.2 (``zai-org/GLM-5.2``) with GSPO and emit vLLM-ready
serving artifacts. GLM 5.2 is a 754B-parameter MoE reasoning model, so
...
def create_glm5_2_dry_run_payload(
    *,
    ...
    preview = create_glm5_2_preview(config)

    serving_recommendations = get_glm5_2_serving_recommendations(
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Dedicated GSPO starter for zai-org/GLM-5.2",
...
Examples:
  ...
  python examples/finetune_glm5_2_gspo.py --model your-org/GLM-5.2-FP8 --fp8-serving --dry-run
  ...
  python examples/finetune_glm5_2_gspo.py --config ./glm5_2.json --no-dry-run
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to a GLM 5.2 starter config file (JSON/YAML).",
    )
    parser.add_argument(
        "--model",
        type=str,
        default=GLM5_2_BASE_MODEL,
        help=(
            "Model name. Use zai-org/GLM-5.2 for BF16 training or "
            "your-org/GLM-5.2-FP8 for FP8 serving previews."
        ),
    )
    parser.add_argument(
        "--task",
        ...
        choices=list(GLM5_2_TASK_CHOICES),
    ...
    parser.add_argument(
        ...
        type=str,
        default="balanced",
        choices=list(GLM5_2_STARTER_PROFILE_CHOICES),
    ...
    parser.add_argument(
        "--fp8-serving",
        action="store_true",
        help="Recommend FP8 single-host serving (your-org/GLM-5.2-FP8) in the manifest.",
    )
    parser.add_argument(
    ...
    if args.config:
        conflicting = []
        if args.model != GLM5_2_BASE_MODEL:
        ...
        resolved_config = load_glm5_2_config_file(args.config)
    else:
        config_overrides: dict[str, Any] = {}
        if args.iterations is not None:
            config_overrides["num_outer_iterations"] = args.iterations
        resolved_config = get_glm5_2_config(
    ...
    if args.write_config:
        written_path = write_glm5_2_config_file(resolved_config, args.write_config)
        logger.info("Wrote GLM 5.2 config to %s", written_path)
        return

    if args.dry_run:
        preview = create_glm5_2_preview(resolved_config)
        serving_recommendations = get_glm5_2_serving_recommendations(
            use_fp8=args.fp8_serving or resolved_config.model_name == GLM5_2_FP8_MODEL,
    ...
    asyncio.run(run_glm5_2_config(resolved_config, dry_run=False))
    logger.info("GLM 5.2 starter run complete")


# Re-export the async finetune helper for direct programmatic use.
__all__ = [
    "create_glm5_2_dry_run_payload",
    "finetune_glm5_2",
    "main",
]
