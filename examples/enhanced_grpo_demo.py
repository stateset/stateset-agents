...
class DemoRewardFunction(RewardFunction):
    ...
    async def compute_reward(self, turns, context=None):
        ...
        response_quality = min(
            1.0, len(last_turn.agent_response) / 100.0
        )  # Longer = better
...
async def demo_training_function(
...
async def demonstrate_adaptive_learning():
    ...
    controller = create_adaptive_learning_controller(
        curriculum_strategy="performance_based", exploration_strategy="curiosity_driven"
    )

    # Simulate training episodes
    task_id = "conversation_task_1"
    current_hyperparams = {"learning_rate": 0.001, "epsilon": 0.1, "temperature": 0.7}
    ...
    for episode in range(10):
        ...
        logger.info(
            f"Episode {episode + 1}: Reward={reward:.3f}, Difficulty={difficulty:.3f}, "
...
async def demonstrate_neural_architecture_search():
    ...
    nas_controller = create_nas_controller(
        strategy="evolutionary", min_depth=2, max_depth=6, min_width=64, max_width=512
    )
    ...
    if optimal_architecture:
        logger.info(
            f"Found optimal architecture with {optimal_architecture.total_parameters} parameters"
        )
        logger.info(f"Performance score: {optimal_architecture.performance_score:.4f}")
        logger.info(
            f"Architecture depth: {optimal_architecture.depth}, width: {optimal_architecture.width}"
        )

    # Get search insights
    insights = nas_controller.get_search_insights()
    logger.info(
        f"NAS evaluated {insights['total_architectures_evaluated']} architectures"
    )
...
async def demonstrate_multimodal_processing():
    ...
    processor = create_multimodal_processor(
        modalities=["text", "structured"], fusion_strategy="attention_fusion"
    )
    ...
    features, metadata = await processor.process_multimodal_input(
        [text_input, structured_input]
    )
...
async def demonstrate_intelligent_orchestration():
    ...
    for step in range(8):
        ...
        logger.info(
            f"Step {step + 1}: Reward={reward:.3f}, Success={success}, "
            f"Decision={decision.action} (confidence={decision.confidence:.2f})"
        )

        # Update hyperparameters if suggested
        if (
            "adaptive_learning" in results
            and "optimized_hyperparams" in results["adaptive_learning"]
        ):
    ...
    logger.info(
        f"Orchestration completed: {status['performance_summary']['total_steps']} steps"
    )
    logger.info(
        f"Recent performance: {status['performance_summary']['recent_performance']:.3f}"
    )
    logger.info(
        f"Performance trend: {status['performance_summary']['performance_trend']:.4f}"
    )
...
async def main():
    ...
    try:
        ...
        (
            multimodal_features,
            multimodal_metadata,
        ) = await demonstrate_multimodal_processing()
        print()
        ...
        logger.info(
            "  • Multimodal Processing for text, images, audio, and structured data"
        )
