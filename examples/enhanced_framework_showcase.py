...
from core.enhanced.enhanced_agent import (
    EnhancedMultiTurnAgent, create_enhanced_agent, create_domain_specific_agent,
    VectorMemory, ReasoningEngine, PersonaProfile
)
from core.enhanced.advanced_rl_algorithms import (
    create_ppo_trainer, create_dpo_trainer, create_a2c_trainer,
    create_advanced_rl_orchestrator, AdvancedRLOrchestrator
)
from core.enhanced.advanced_evaluation import (
    AdvancedEvaluator, AutomatedTestSuite, ComparativeAnalyzer,
    create_evaluation_config, quick_agent_comparison
)
from core.environment import ConversationEnvironment, TaskEnvironment
from core.reward import create_domain_reward, create_helpful_agent_reward
...
class EnhancedFrameworkDemo:
    ...
    async def demonstrate_agent_capabilities(self):
        ...
        messages = [{"role": "user", "content": "Hello! Can you help me understand how machine learning works?"}]
        response = await agent.generate_response(messages)
        logger.info(f"🤖 Agent Response: {response[:200]}...")
        
        # Test memory capabilities
        logger.info("\nTesting memory capabilities...")
        memory_content = "The user is interested in machine learning and wants a simple explanation."
        ...
        follow_up = [{"role": "user", "content": "Can you give me a practical example of this?"}]
        response = await agent.generate_response(follow_up)
        logger.info(f"🤖 Memory-informed response: {response[:200]}...")
        
        # Test reasoning capabilities
        logger.info("\nTesting reasoning capabilities...")
        reasoning_query = [{"role": "user", "content": "Should I use Python or Java for my web application? Consider factors like development speed, performance, and community support."}]
        response = await agent.generate_response(reasoning_query)
        logger.info(f"🤖 Reasoning response: {response[:200]}...")
        
        # Show agent status
        status = agent.get_agent_status()
        logger.info(f"📊 Agent Status: {status['memory_entries']} memories, {status['reasoning_sessions']} reasoning sessions")
    ...
    async def demonstrate_rl_algorithms(self):
        ...
        logger.info("RL Orchestrator would select the best algorithm based on data characteristics")
    ...
    async def demonstrate_evaluation_framework(self):
        ...
        for test_name, result in list(test_results.items())[:3]:
            ...
            logger.info(f"Test '{test_name}': Pass rate = {summary['pass_rate']:.2f}, Avg time = {summary['avg_execution_time']:.3f}s")
    ...
    async def run_performance_benchmark(self):
        ...
        logger.info("📈 Performance Results:"        logger.info(f"  Average response time: {avg_time:.3f} ± {std_time:.3f} seconds")
    ...
    async def showcase_domain_specific_agents(self):
        ...
        cs_scenario = [
            {"role": "user", "content": "Hi, I ordered a laptop last week but it hasn't arrived yet. I'm really frustrated!"}
        ]
        ...
        tech_scenario = [
            {"role": "user", "content": "My computer keeps freezing when I try to run this software. What should I do?"}
        ]
    ...
    async def run_full_demo(self):
        ...
        try:
            ...
            logger.info("")
            logger.info("🚀 Your AI Agents RL framework is now significantly more powerful!")
