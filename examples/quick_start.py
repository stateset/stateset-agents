...
async def test_conversation(agent):
    ...
    conversation = [{"role": "system", "content": agent.config.system_prompt}]
...
async def custom_reward_example():
        ...
        async def compute_reward(self, turns, context=None):
            ...
            for i, turn in enumerate(assistant_turns):
                ...
                politeness_count = sum(
                    1 for phrase in polite_phrases if phrase in content_lower
                )
    ...
    scenarios = [
        {
            "id": "polite_test",
            "user_responses": ["Hello!", "Can you help me?", "Thank you!"],
        }
    ]
    ...
    reward_fn = CompositeReward(
        [PolitenesReward(weight=0.6), HelpfulnessReward(weight=0.4)]
    )
