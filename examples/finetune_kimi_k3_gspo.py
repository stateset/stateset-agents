"""Dedicated Kimi-K3 GSPO starter script."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Dedicated GSPO starter for moonshotai/Kimi-K3",
...
Examples:
  ...
  python examples/finetune_kimi_k3_gspo.py --write-config ./kimi_k3.json

  # Run from a saved config
  python examples/finetune_kimi_k3_gspo.py --config ./kimi_k3.json --no-dry-run
        """,
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        choices=list(KIMI_K3_TASK_CHOICES),
    ...
    parser.add_argument(
        ...
        type=str,
        default="balanced",
        choices=list(KIMI_K3_STARTER_PROFILE_CHOICES),
    ...
    if args.list_profiles:
        ...
        payload = describe_kimi_k3_starter_profiles(task=args.task, model_name=args.model)
        print(json.dumps(payload, indent=2, default=str))
        return

    if args.config:
        conflicting = []
        if args.model != KIMI_K3_BASE_MODEL:
        ...
        resolved_config = load_kimi_k3_config_file(args.config)
    else:
        config_overrides = {}
        if args.iterations is not None:
    ...
    if args.write_config:
        written_path = write_kimi_k3_config_file(resolved_config, args.write_config)
    ...
    asyncio.run(run_kimi_k3_config(resolved_config, dry_run=False))
    logger.info("Kimi-K3 starter run complete")
