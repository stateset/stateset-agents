...
async def create_enhanced_customer_service_agent(
    ...
    config = AgentConfig(
        ...
        peft_config={
            "r": 8,
            "lora_alpha": 16,
            "target_modules": ["q_proj", "v_proj"],  # Will be auto-detected based on model
            "lora_dropout": 0.05,
            "bias": "none",
            "task_type": "CAUSAL_LM"
...
async def prepare_training_data(
    ...
    logger.info(f"Data prepared: {len(train_data)} train, {len(eval_data)} eval examples")
...
async def main():
    ...
    try:
        ...
        if config.run_post_eval and eval_scenarios:
            ...
            logger.info(f"  Overall mean reward: {eval_results['overall_stats']['overall_mean_reward']:.4f}")
            logger.info(f"  Reward distribution:")
            for percentile, value in eval_results['overall_stats']['reward_distribution'].items():
        ...
        if input("\nWould you like to test the trained agent interactively? (y/n): ").lower() == 'y':
