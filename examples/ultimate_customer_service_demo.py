...
class AdvancedCustomerServiceAgent:
    ...
    def _register_tools(self):
        ...
        async def search_knowledge_base(query: str) -> str:
            ...
            if relevant_dialogues:
                return f"Found {len(relevant_dialogues)} relevant articles: " + \
                       ", ".join([d.get("category", "general") for d in relevant_dialogues])
            else:
        ...
        async def create_return_label(order_number: str) -> str:
            ...
            return f"Return label created for order {order_number}. " + \
                   "You'll receive an email with the prepaid return label within 5 minutes."
        
        async def escalate_to_human(reason: str) -> str:
            """Escalate to human agent"""
            return f"I'm escalating your case to a human specialist for: {reason}. " + \
        ...
        try:
            ...
            if conversation_id:
                ...
                conversation_context = await self.agent.start_conversation(
                    user_id=customer_context.get("user_id") if customer_context else None,
                    initial_context={
                        "topic": "customer_service",
                        "priority": customer_context.get("priority", "medium") if customer_context else "medium",
            ...
            if reward_results:
                average_reward = sum(r.score for r in reward_results) / len(reward_results)
                self.neural_reward.add_trajectory_feedback(turns, average_reward, context)
            ...
            return {
                ...
                "metrics": {
                    ...
                    "average_quality": sum(r.score for r in reward_results) / len(reward_results) if reward_results else 0.0
                },
                "suggested_actions": self._get_suggested_actions(message, response, context)
            }
    ...
    def _get_suggested_actions(self, message: str, response: str, context: Dict[str, Any]) -> List[str]:
    ...
    async def run_training_simulation(self, num_iterations: int = 10):
        ...
        logger.info(f"Total computation used: {final_metrics['computation_used']:.2f} seconds")
        logger.info(f"Total trajectories: {final_metrics['total_trajectories']}")
        logger.info(f"Average trajectories per second: {final_metrics['engine_metrics']['trajectories_per_second']:.2f}")
        ...
        return {
            ...
            "performance_summary": {
                ...
                "computation_efficiency": final_metrics["engine_metrics"]["trajectories_per_second"]
            }
        }
    ...
    async def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dialogue_database_stats": {
                ...
                "categories": list(set(d.get("category", "general") for d in self.dialogue_db.dialogues))
            }
        }
...
async def run_interactive_demo():
    ...
    print("Type 'quit' to exit, 'metrics' to see performance, 'train' to run training simulation")
    print("-" * 40)
    ...
    while True:
        try:
            ...
            if user_input.lower() == 'quit':
                ...
                print(f"Training completed! Average reward: {training_results['performance_summary']['avg_reward']:.3f}")
                continue
...
async def run_benchmark_demo():
    ...
    for i, (scenario, message) in enumerate(test_scenarios):
        ...
        result = await agent.handle_customer_interaction(
            message,
            customer_context={"priority": "medium", "scenario": scenario}
        )
    ...
    print(f"⚡ Fastest response: {fastest_response['scenario']} ({fastest_response['response_time']:.2f}s)")
