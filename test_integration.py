...
class IntegrationTester:
    ...
    async def test_model_checkpoint_saving_loading(self):
        ...
        try:
            ...
            assert (checkpoint_path / "pytorch_model.bin").exists() or \
                   (checkpoint_path / "model.safetensors").exists(), "Model file not saved"
            ...
            assert loaded_agent.tokenizer is not None, "Tokenizer not loaded from checkpoint"
    ...
    def test_wandb_integration(self):
        ...
        try:
            ...
            if os.getenv("WANDB_API_KEY"):
                ...
                self.log_test_result(
                    "W&B live integration",
                    True,
                    "Successfully logged to W&B"
                )
            else:
                self.log_test_result(
                    "W&B live integration",
                    False,
                    "WANDB_API_KEY not available"
                )
    ...
    def test_training_config(self):
        ...
        try:
            ...
            assert loaded_config.learning_rate == config.learning_rate, "Config loading failed"
    ...
    def print_summary(self):
        ...
        hf_working = any(
            "HuggingFace" in r["test"] and r["success"] 
            for r in self.test_results
        )
        
        config_working = any(
            "configuration" in r["test"] and r["success"]
            for r in self.test_results
        )
        
        training_working = any(
            "end-to-end" in r["test"] and r["success"]
            for r in self.test_results
        )
