...
class RulerRewardFunction(RewardFunction):
    ...
    def __init__(
        ...
        if not LITELLM_AVAILABLE:
            raise ImportError("litellm is required for LLM Judge reward function. Install with: pip install litellm")
    ...
    def _heuristic_fallback(self, turns: List[Dict[str, Any]]) -> float:
        ...
        # Quality indicators
        helpful_phrases = [
            "i can help", "let me", "would you like", "please",
            "here's", "you can", "try", "suggest", "recommend"
        ]
    ...
    def create_custom_rubric(
        ...
        rubric_lines.append("Give higher scores to responses that excel in multiple categories.")
        return "\n".join(rubric_lines)
